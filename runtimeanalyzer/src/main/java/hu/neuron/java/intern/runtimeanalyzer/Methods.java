...
public class Methods {
	public static Long[] setMethodBlock() {
		...
		Long[] times = setMethods(methodsList, parametersList, implementsList);
		return times;
	}

	public static Long[] setMethods(List methodsList, List parametersList, List implementsList) {
			...
			for (int i = 0; i < methodsList.size(); i++) {
				...
				try {
					...
					if (parametersList.get(i).toString().equals("Object")) {
						...
						time[index] = timeKeeper.timeKeeper(method, params, c);
						...
						time[index] = timeKeeper.timeKeeper(method, c);
					} else if (parametersList.get(i).toString().equals("intObject")) {
						params = int.class;
						params2 = Object.class;
						method = Class.forName(object.toString()).getMethod(methodsList.get(i).toString(), params,
								params2);
						time[index] = timeKeeper.timeKeeper(method, params, params2, c);
